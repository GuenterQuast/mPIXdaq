...
class frameAnalyzer:
    ...
    def cluster_properties(self, f, pl):
        ...
        Returns:
            ...
            x_mn, y_mn, w, h : minimum x, y and width and height of bounding box
    ...
    def write_csv(file, timestamp, clusters):
        ...
        for _xym, _npix, _energy, _e_mx, _bbox, _var, _angle, _xyEm, _varE in clusters:
            if _npix == 0:
                return
            print(
                f"{timestamp:.3f}, {_xym[0]:.2f}, {_xym[1]:.2f}, {_npix}, {_energy:.1f}, {_e_mx:.1f}"
                + f", {_bbox[0]:.1f}, {_bbox[1]:.1f}, {_bbox[2]:.1f}, {_bbox[3]:.1f}"
    ...
    def write_csvheader(file):
        ...
        csvHeader = (
            "time,x_mean,y_mean,n_pix,energy,e_mx,x_mn,y_mn,w,h,var_mx,var_mn,angle,xE_mean,yE_mean,varE_mx,varE_mn"
        )
    ...
    def get_cluster_summary(pixel_clusters):
        """summarize cluster properties from cluster tuples of format
              ( (x,y), n_pix, energy, e_mx, (x_mn, y_mn, w, h), (var_mx, var_mn), angle, (xE, yE), (varE_mx, VarE_mn))
...
class mpixGraphs:
    ...
    def __init__(self, nover=10, unit='keV', circ=0.5, flat=0.5, acq_time=1.0, prescale=1):
        ...
        self.N_clusters = 0
        self.ClusterEnergy = 0.0
        self.N_singles = 0
        self.SingleEnergy = 0.0
    ...
    def __call__(self, frame2d, c_summary, dt_active, dt_alive):
        ...
        #  adjust axis
        if _mx_rate > self.rate_mx:
            self.rate_mx = _mx_rate
            self.axRate.set_xlim(-0.1, 1.05 * self.rate_mx)
        elif 2 * _mx_rate < self.rate_mx:
            self.rate_mx = _mx_rate / 2
            self.axRate.set_xlim(-0.1, self.rate_mx)
        ...
        if self.i_buf < self.n_overlay - 1:
            ...
            pixel_energies = self.cimage[self.cimage > 0]
            self.upd_histograms(pixel_energies, summary)

            self.N_clusters = self.n_clusters
            self.ClusterEnergy = self.cluster_energies.sum()
...
class runDAQ:
    ...
    def read_frames_from_file(self):
        ...
        suffixes_list_mode = {'.yml', '.txt', '.clog'}
        if suffix in suffixes_list_mode or suffix2 in suffixes_list_mode:
            self.read_mode = 'list'
